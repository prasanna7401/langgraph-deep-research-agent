...
"question_to_user": "<question to ask the user to clarify the report scope>",
"verification": "<verification message that we will start research>"

If you need to ask a clarifying question, return:
"need_clarification": true,
"question_to_user": "<your clarifying question>",
"verification": ""
...
revise_research_brief_prompt = """You previously generated the following research brief, and a human reviewer has asked for refinements.
Your job is to produce an updated research brief that incorporates the reviewer's feedback while preserving everything that was already correct.
...
Guidelines:
1. Minimal, targeted edits
- Tweak the previous brief to address the reviewer's feedback. Do not regenerate the brief from scratch.
- Preserve all detail, scope, constraints, and source preferences that the reviewer did not ask to change.

2. Honor the original brief-writing rules
- Maximize specificity and detail; explicitly list known user preferences and key dimensions.
- Handle unstated dimensions as open considerations, not assumed preferences.
- Never invent user preferences, constraints, or requirements that were not stated in the feedback or original messages.
- Distinguish research scope (what to investigate) from user preferences (only what the user/reviewer stated).
- Phrase the request in the first person, from the user's perspective.

3. Sources
- Keep any source preferences from the previous brief unless the reviewer explicitly overrides them.
- Apply any new source preferences from the reviewer (e.g., peer-reviewed sources, specific time windows, official documentation).

Return only the revised research brief - no preamble, no explanation of changes.
"""
...
You can use any of the tools provided to you to find resources that can help answer the research question. Your research is conducted in a tool-calling loop.
</Task>

<Available Tools>
You have access to two tools with very different roles:

1. **tavily_search** - The ONLY tool that fetches real information from the web. This is your primary tool. To answer the user, you MUST call this tool. No facts, URLs, or sources exist in this conversation unless tavily_search produced them.
2. **think_tool** - A reflection-only scratchpad. It returns your own input verbatim. It does NOT search, fetch, browse, or access any external system. It is useless before any search has happened.

**CRITICAL RULES**
- Your VERY FIRST tool call in this conversation MUST be `tavily_search`. Do not emit any `think_tool` call before at least one `tavily_search` has returned results.
- `think_tool` is ONLY allowed AFTER a `tavily_search` ToolMessage exists in the conversation, to reflect on those results.
- Never use `think_tool` to "announce" or "plan" upcoming searches. If you want to run searches, call `tavily_search` directly.
- Do not emit `tavily_search` and `think_tool` in the same parallel tool-call batch. Search first, observe results, then reflect.
</Available Tools>
...
2. **Issue a real search FIRST** - Call `tavily_search` with a broad, comprehensive query before doing anything else.
3. **After each search, pause and assess** - Use `think_tool` to reflect on actual returned results: what was found, what's missing, what to query next.
4. **Execute narrower searches** - Call `tavily_search` again to fill gaps.
5. **Stop when you can answer confidently** - Don't keep searching for perfection.
</Instructions>

<Hard Limits>
**Tool Call Budgets** (Prevent excessive searching):
- **Simple queries**: Use 2-3 `tavily_search` calls maximum
- **Complex queries**: Use up to 5 `tavily_search` calls maximum
- **Always stop**: After 5 `tavily_search` calls if you cannot find the right sources
- `think_tool` calls do not count toward the search budget, but each must follow a real search.

**Stop Immediately When**:
- You can answer the user's question comprehensively from real search results
- You have 3+ relevant sources for the question
- Your last 2 searches returned similar information
</Hard Limits>

<Show Your Thinking>
AFTER each `tavily_search` returns, use `think_tool` to analyze the actual results:
- What key information did I find in the returned sources?
- What's missing?
- Do I have enough to answer the question comprehensively?
- Should I search more or provide my answer?

Never use `think_tool` to describe searches you have not yet executed.
</Show Your Thinking>
"""
...
{{
   ...
   "key_points": "First important quote or excerpt, Second important quote or excerpt, Third important quote or excerpt, ...Add more excerpts as needed, up to a maximum of 5"
}}
```
...
{{
   ...
   "key_points": "Our findings indicate a clear acceleration in sea-level rise, which has significant implications for coastal planning and adaptation strategies, lead author Dr. Emily Brown stated. The rate of ice sheet melt in Greenland and Antarctica has tripled since the 1990s, the study reports. Without immediate and substantial reductions in greenhouse gas emissions, we are looking at potentially catastrophic sea-level rise by the end of this century, warned co-author Professor Michael Green."  
}}
```
